...
import {
  getGenreMovieResults,
  getGenreShowResults,
  getMoreGenreMovieResults,
  getMoreGenreShowResults,
...
export const loadMoreMovieGenreResults =
  (genres: Genre[], page: number) => async (dispatch: Dispatch) => {
    dispatch(loading());
    const genreIds = genres.map((genre) => genre.id);
    const genresAsString = genreIds.join('%2C');
...
export const loadMoreShowGenreResults =
  (genres: Genre[], page: number) => async (dispatch: Dispatch) => {
    dispatch(loading());
    const genreIds = genres.map((genre) => genre.id);
    const genresAsString = genreIds.join('%2C');
...
export const loadSearchResults =
  ...
  async (dispatch: Dispatch) => {
    ...
    const detailedResponse = results
